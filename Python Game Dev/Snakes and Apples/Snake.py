...
def gameloop():
    ...
    velocity_x = 0
    velocity_y = 0
    food_x = random.randint(20, set_width/1.5)
    food_y = random.randint(20, set_height/1.5)
    score = 0
    init_velocity = 4
    snake_list = []
    snake_length = 1
    ...
    while not exit_game:
        ...
        if game_over:
            ...
            # Condtition to eat the food
            if abs(snake_x - food_x) < 20 and abs(snake_y - food_y) < 20:
                score += 10
                pygame.mixer.music.load("Eating sound.wav")
                pygame.mixer.music.play()

                food_x = random.randint(0, set_width)
                food_y = random.randint(0, set_height)
